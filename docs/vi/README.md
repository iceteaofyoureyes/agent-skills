# Tài liệu BA Kit — Tiếng Việt

Tiếng Việt là ngôn ngữ chính của tài liệu người dùng. Các trang tiếng Anh trong [English documentation](../en/README.md) là bản tương đương để tham khảo; nếu hai bản lệch nghĩa, hãy đồng bộ lại theo bản tiếng Việt.

## Bắt đầu với BA Kit

1. [Hướng dẫn nhanh](BA_KIT_QUICKSTART.md)
2. [Cài đặt](INSTALLATION.md)
3. [Quy trình và Human Gate](BA_KIT_WORKFLOW.md)
4. [Hướng dẫn sử dụng](BA_KIT_USAGE_GUIDE.md)
5. [Ví dụ CR-001](../../kits/ba/examples/CR-001/README.md)
6. [Câu hỏi thường gặp](BA_KIT_FAQ.md)

## Tài liệu tham khảo

- [Tổng quan kiến trúc](ARCHITECTURE.md)
- [Nền tảng thiết kế, chuẩn tham chiếu và design lineage](FOUNDATIONS.md)
- [Hợp đồng của Kit](KIT_CONTRACT.md)
- [Trạng thái phát hành](RELEASE.md)
- [Nguồn gốc và giấy phép](PROVENANCE.md)

---

English: [Documentation index](../en/README.md)
