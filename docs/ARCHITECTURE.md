...
This repository keeps every atomic skill canonical at its root. `kits/ba/kit.yaml` composes the BA role, and `core/manifest.yaml` classifies shared capabilities without moving or duplicating their folders. BA workflow orchestration lives in `ba-workflow/`; detailed procedures remain in atomic skills.
