...
Required fields and policy are in `templates/engineering-handoff.yml`. The validator accepts the package's documented YAML subset (nested mappings and inline or block lists), checks all required fields and SHA-256 syntax, and when given a file verifies that each relative authoritative-source path exists and matches its recorded digest. A handoff with unresolved blocking items or without `APPROVED_FOR_ENGINEERING` must not be presented as ready.
