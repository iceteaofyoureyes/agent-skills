...
**Related Skills**
PDF, spreadsheet, presentation and codebase-writing skills are separate capabilities and are not bundled in BA Kit. Use them only when independently installed in the target agent environment.
...
If `learnings.consolidated.md` or `learnings.md` exists in this skill directory, read it before a non-trivial task. These local usage notes are optional and are not distributed with BA Kit.

After applying it, record durable learnings only when the host project provides an approved learning workflow. Do not require `agents-skills-feedback-loop` or modify this upstream skill as a side effect.
